      ...
      <Collapsible open={posCardOpen} onOpenChange={setPosCardOpen}>
        <Card>
          ...
          <div className="p-4 rounded-lg border mt-4">
            ...
            {/* SepetTakip Loglar Bölümü */}
            {sepettakipData?.has_credentials && (
              <div className="mt-4 border-t pt-4">
                <Button 
                  size="sm" 
                  variant="outline"
                  onClick={() => { setShowLogs(!showLogs); if (!showLogs) { fetchSepettakipLogs(); } }}
                  className="w-full bg-slate-50 hover:bg-slate-100 border-slate-200"
                >
                  {showLogs ? "Logları Kapat" : "📋 Gelen İstek Logları"}
                </Button>
                
                {showLogs && (
                  <div className="mt-4 space-y-3 p-4 bg-slate-900 rounded-lg border text-white">
                    <div className="flex justify-between items-center">
                      <h4 className="font-medium text-sm text-slate-200">SepetTakip Logları</h4>
                      <div className="flex gap-2">
                        <Button size="sm" variant="ghost" onClick={fetchSepettakipLogs} className="text-xs text-slate-300 hover:text-white">
                          {loadingLogs ? <RefreshCw className="w-3 h-3 animate-spin" /> : "Yenile"}
                        </Button>
                        <Button size="sm" variant="ghost" onClick={copyLogsToClipboard} className="text-xs text-slate-300 hover:text-white">
                          Kopyala
                        </Button>
                        <Button size="sm" variant="ghost" onClick={clearSepettakipLogs} className="text-xs text-red-400 hover:text-red-300">
                          Temizle
                        </Button>
                      </div>
                    </div>
                    ...
                    ) : sepettakipLogs.length === 0 ? (
                      <div className="text-center py-6 text-slate-400 text-sm">
                        Henüz log kaydı yok.<br/>
                        <span className="text-xs">SepetTakip'ten istek geldiğinde burada görünecek.</span>
                      </div>
                    ) : (
                      <div className="max-h-80 overflow-y-auto space-y-2">
                        {sepettakipLogs.map((log, idx) => (
                          <div key={idx} className={`p-3 rounded text-xs font-mono ${
                            ...
                            <div className="flex justify-between text-slate-400 mb-1">
                              <span className={`font-semibold ${
                                ...
                                log.type?.includes('create') ? 'text-green-400' : 'text-blue-400'
                              }`}>
                                {log.type || 'LOG'}
                              </span>
                              <span>{log.timestamp ? new Date(log.timestamp).toLocaleString('tr-TR') : ''}</span>
                            </div>
                            {log.order_id && (
                              <div className="text-slate-300">Order ID: <span className="text-yellow-400">{log.order_id}</span></div>
                            )}
                            {log.status_code && (
                              <div className="text-slate-300">Status: <span className={log.status_code >= 400 ? 'text-red-400' : 'text-green-400'}>{log.status_code}</span></div>
                            )}
                            {log.error && (
                              <div className="text-red-400 mt-1">Hata: {log.error}</div>
                            )}
                            {log.body && (
                              <details className="mt-2">
                                <summary className="cursor-pointer text-slate-400 hover:text-slate-200">Request Body</summary>
                                <pre className="mt-1 p-2 bg-slate-950 rounded overflow-x-auto text-slate-300 whitespace-pre-wrap break-all">
                                  {typeof log.body === 'string' ? log.body.substring(0, 1000) : JSON.stringify(log.body, null, 2).substring(0, 1000)}
                                </pre>
                              </details>
                            )}
                            {log.response && (
                              <details className="mt-2">
                                <summary className="cursor-pointer text-slate-400 hover:text-slate-200">Response</summary>
                                <pre className="mt-1 p-2 bg-slate-950 rounded overflow-x-auto text-slate-300 whitespace-pre-wrap break-all">
                                  {typeof log.response === 'string' ? log.response.substring(0, 1000) : JSON.stringify(log.response, null, 2).substring(0, 1000)}
                                </pre>
                              </details>
                            )}
                          </div>
