    ...
    <div className="rounded-lg border border-slate-200 bg-white overflow-hidden" data-testid={`active-order-card-${order.id}`}>
      ...
      <div className={`${sc.color} px-3 py-1.5 flex items-center justify-between`}>
        <span className="text-[11px] font-bold text-white flex items-center gap-1">
          {sc.label}
          {age && (isLate
            ? <span className="animate-pulse flex items-center"><span className="text-white">·</span> <span className="text-red-500 font-extrabold">!</span> <span className="text-red-500 font-extrabold">{age}</span></span>
            : <span>· {age}</span>
          )}
        </span>
