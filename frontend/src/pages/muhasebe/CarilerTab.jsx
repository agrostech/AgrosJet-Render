    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            vendors.map((vendor) => (
              <div
                ...
                className={`flex items-center gap-3 p-3 border-b border-slate-100 transition-colors ${
                  selectedVendor?.id === vendor.id ? "bg-primary/10 border-l-4 border-l-primary" : "hover:bg-slate-50"
                }`}
              >
                <button onClick={() => setSelectedVendor(vendor)} className="flex-1 flex items-center gap-3 text-left">
                ...
                <button onClick={() => handleDeleteVendor(vendor.id)} className="text-red-400 hover:text-red-600 p-1">
      ...
      <Dialog open={showAddModal} onOpenChange={setShowAddModal}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle className="font-heading">Cari Ekle</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleAddVendor} className="space-y-4">
            <div>
              <Label className="text-sm font-semibold">Cari Adı *</Label>
              <Input value={newVendor.name} onChange={(e) => setNewVendor({ ...newVendor, name: e.target.value })} className="mt-1 h-10 border-2" required />
            </div>
            <div>
              <Label className="text-sm font-semibold">Telefon</Label>
              <Input value={newVendor.phone} onChange={(e) => setNewVendor({ ...newVendor, phone: e.target.value })} className="mt-1 h-10 border-2" />
            </div>
            <div>
              <Label className="text-sm font-semibold">Adres</Label>
              <Input value={newVendor.address} onChange={(e) => setNewVendor({ ...newVendor, address: e.target.value })} className="mt-1 h-10 border-2" />
            </div>
