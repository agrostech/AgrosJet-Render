...
- 2026-02: **Adisyo entegrasyonu için zenginleştirilmiş API log sistemi**. `services/adisyo_service.py` içinde `mark_adisyo_order_prepared`, `mark_adisyo_order_on_delivery`, `mark_adisyo_order_delivered`, `cancel_adisyo_order` fonksiyonları her çağrıda `save_integration_log("adisyo", ...)` ile MongoDB `integration_logs` koleksiyonuna yazıyor: endpoint, restaurant_id, adisyo_order_id, request_body, response_status_code, response_body, exception bilgileri dahil. `GET /api/integration-logs` endpoint'i `data` alanını da döndürüyor (`routers/admins.py`). Frontend `components/admin/IntegrationLogsModal.jsx` her log satırı altında "İstek/Yanıt detayı" expandable details ile JSON gösteriyor. Yönetim paneli → Restoranlar → Entegrasyon Logları → Adisyo sekmesinden Adisyo'nun gerçek API yanıtları izlenebilir. Hot reload uyumlu, ek deps yok. 1 günlük gözlem sonrası kurye teslim → Adisyo "yolda" sorununu kök nedene indirgeyeceğiz.
