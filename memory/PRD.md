...
- 2026-02-18: **Otomatik İşleme E-posta Raporları**. 3 farklı scheduler (Kurye Hakediş Günlük, Kurye Faturaları Haftalık Yükümlülük, Restoran Faturaları Haftalık) her çalıştığında şirketin tüm süperadminlerine HTML rapor e-postası gönderiyor (`services/email_service.py:send_auto_process_report` + `get_all_superadmin_emails`). İçerik: şirket adı, dönem, çalışma zamanı, başarılı işlem sayısı/toplam tutar, atlanan/hatalı kayıtlar listesi (kurye/restoran adı + sebep). Sistem SMTP ayarları (`system_settings.type=smtp`) kullanılır. Auto-process **enable** olmayan veya o gün toggle kapalı şirketlere mail gitmez (gereksiz gürültü olmaz). Değişen dosyalar: `services/email_service.py`, `services/daily_hakedis_service.py` (apply_day_hakedis processed/skipped'a courier_name ekledi), `routers/courier_invoice_obligations.py` (`generate_weekly_obligations_for_company` artık `{enabled, created, items, skipped, week_start, week_end}` döndürüyor), `routers/restaurant_invoices.py` (`generate_weekly_missing_invoices` artık `items + week_label` döndürüyor), `server.py` (3 scheduler job email çağrısı). Test: HTML/plain render doğrulandı, SMTP send akışı izlendi.

- 2026-02-18: **Manuel fatura artık predicted'i düşürmüyor + haftalık badge sayacı düzeldi** (2 bug fix). `routers/courier_invoice_obligations.py`:
  - `by_week`: `courier_existing_total` artık `is_manual` faturaları toplamıyor (sadece auto, non-remainder obligationları sayıyor). Sonuç: kurye için manuel 200 TL fatura oluşturulsa bile, haftalık predicted satırı tam hakediş tutarını gösteriyor; manuel kendi bağımsız satırı olarak listede ayrı duruyor.
