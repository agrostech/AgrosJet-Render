...
async def mark_trendyol_order_ready(restaurant_id: str, order_id: str) -> dict:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=15.0) as client:
            ...
            if response.status_code == 200:
                await ilog.info(f"Trendyol sipariş {package_id} hazır olarak işaretlendi")
                return {"success": True, "message": "Sipariş hazır olarak işaretlendi"}
            else:
                error_detail = _extract_error(response)
                await ilog.warning(f"Trendyol hazır hatası: {response.status_code} - {error_detail}")
                return {"success": False, "error": f"API hatası: {response.status_code} - {error_detail}"}
                
    except Exception as e:
        await ilog.exception("Trendyol hazır hatası")
        return {"success": False, "error": str(e)}
...
async def mark_trendyol_order_shipped(restaurant_id: str, order_id: str) -> dict:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=15.0) as client:
            ...
            if response.status_code == 200:
                await ilog.info(f"Trendyol sipariş {package_id} yola çıktı")
                return {"success": True, "message": "Sipariş yola çıktı olarak işaretlendi"}
            else:
                error_detail = _extract_error(response)
                await ilog.warning(f"Trendyol yola çıktı hatası: {response.status_code} - {error_detail}")
                return {"success": False, "error": f"API hatası: {response.status_code} - {error_detail}"}
                
    except Exception as e:
        await ilog.exception("Trendyol yola çıktı hatası")
        return {"success": False, "error": str(e)}
